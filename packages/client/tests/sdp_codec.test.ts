...
describe("encodeSdp / decodeSdp round-trips", () => {
  it("round-trips a Chrome offer with IPv4 host + srflx candidates", () => {
    const packet = encodeSdp(CHROME_OFFER_SDP, false, "test-room", "AAAAAAAAAA8");
  ...
  it("round-trips a Chrome answer (is_answer flag)", () => {
    const packet = encodeSdp(CHROME_ANSWER_SDP, true, "test-room", "AAAAAAAAAA8");
    const { sdp, type } = decodeSdp(packet);
  ...
  it("round-trips an SDP with an IPv6 host candidate", () => {
    const packet = encodeSdp(IPV6_SDP, false, "test-room", "AAAAAAAAAA8");
  ...
  it("round-trips an SDP with an mDNS candidate", () => {
    const packet = encodeSdp(MDNS_SDP, false, "test-room", "AAAAAAAAAA8");
    const { sdp } = decodeSdp(packet);
...
describe("encodeSdp packet structure", () => {
  ...
  it("has version byte 0x03", () => {
    const packet = encodeSdp(CHROME_OFFER_SDP, false, "test-room", "AAAAAAAAAA8");
    expect(packet[1]).toBe(0x03);
  });

  it("sets the is_answer flag for answers", () => {
    const offer_packet = encodeSdp(CHROME_OFFER_SDP, false, "test-room", "AAAAAAAAAA8");
    const answer_packet = encodeSdp(CHROME_ANSWER_SDP, true, "test-room", "AAAAAAAAAA8");
  ...
  it("produces the expected 96-byte packet for 2 IPv4 candidates with a 9-byte room_id", () => {
    // 3 (header) + 32 (fp) + 1 + 4 (ufrag) + 1 + 22 (pwd) + 1 + 7 + 7 (candidates)
    // + 1 (room_id length) + 9 ("test-room") + 8 (room_token) = 96
    const packet = encodeSdp(CHROME_OFFER_SDP, false, "test-room", "AAAAAAAAAA8");
    expect(packet.byteLength).toBe(96);
  });
});
...
describe("candidate filtering", () => {
  it("excludes relay candidates", () => {
    ...
    const packet = encodeSdp(sdp_with_relay, false, "test-room", "AAAAAAAAAA8");
    const { sdp } = decodeSdp(packet);
    const candidate_count = (sdp.match(/a=candidate:/g) ?? []).length;
  ...
  it("limits to 1 host IPv4 candidate even if multiple are present", () => {
    ...
    const packet = encodeSdp(sdp_multi, false, "test-room", "AAAAAAAAAA8");
    const { sdp } = decodeSdp(packet);
